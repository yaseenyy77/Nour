...
const ProductManagement = ({ onBack }) => {
  ...
  // دالة الحذف (المعدلة لإجبار التحقق من قاعدة البيانات)
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this royal piece?')) {
      try {
        // نرسل أمر الحذف لسوبابيز أولاً
        ...
        if (error) {
          // لو سوبابيز رفض الحذف (بسبب الـ RLS مثلاً) هيظهر الخطأ هنا
          console.error("Supabase Delete Error:", error.message);
          alert("Database Error: " + error.message);
        } else {
          // لو الحذف نجح فعلياً في القاعدة، نمسحه من الواجهة
  ...
  return (
    <div className="w-full animate-in fade-in duration-500 font-sans" dir="ltr">
      {/* Header Section */}
        ...
        <div className="bg-white rounded-[2.5rem] border-2 border-gray-100 shadow-2xl overflow-hidden">
          {loading ? (
            <div className="p-32 flex flex-col items-center justify-center text-[#123456]">
              ...
              <p className="font-black tracking-[0.2em] animate-pulse">LOADING VAULT...</p>
            </div>
          ) : (
                    ...
                    <tr key={p.id} className="hover:bg-blue-50/40 transition-colors group">
                      <td className="p-6">
                        <img 
                          ...
                          alt={p.name} 
                          className="w-20 h-20 object-cover rounded-2xl shadow-md border-2 border-white group-hover:scale-110 transition-transform duration-300"
                        />
                      </td>
                      <td className="p-6">
                        <div className="font-black text-[#123456] text-xl mb-1">{p.name}</div>
                        <span className="px-3 py-1 bg-gray-100 text-[#446688] text-[10px] font-black rounded-full uppercase tracking-tighter">
                          {p.category}
                        </span>
                      </td>
                      <td className="p-6">
                        <div className="flex flex-col">
                          <span className="text-[#123456] font-bold">{p.karat} Karat</span>
                          <span className="text-gray-400 font-mono text-sm">{p.weight}g</span>
                        </div>
                      </td>
                      <td className="p-6">
                        <div className="flex justify-center">
                          <button 
                            ...
                            className="p-4 text-red-500 hover:bg-red-50 rounded-2xl transition-all hover:rotate-12 active:scale-75"
                          >
                            <Trash2 size={24} />
                          </button>
                        </div>
          ...
          {!loading && products.length === 0 && (
            <div className="p-32 text-center">
              ...
              <p className="text-gray-400 italic text-xl font-light tracking-widest uppercase">The collection is currently empty.</p>
            </div>
          )}
        </div>
      )}
