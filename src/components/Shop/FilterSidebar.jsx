  ...
  return (
    ...
    >
      <div className={`w-[320px] h-full overflow-y-auto px-6 py-4 custom-sidebar-scroll ${!isOpen && 'hidden'}`}>
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          {Object.values(selectedFilters).flat().length > 0 && (
            <button 
              ...
              className="text-[#d4af37] hover:text-[#001b44] transition-colors flex items-center gap-1 text-[9px] font-bold uppercase"
            >
        ...
        {filterCategories.map((cat) => (
          <div key={cat.id} className="mb-10">
            <div className="flex items-center justify-between mb-5 border-b border-gray-50 pb-2">
              <h3 className="text-[#001b44] font-bold text-[11px] uppercase tracking-widest">{cat.title}</h3>
            ...
            <div className="space-y-3">
              {cat.options.map((option) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-5 h-5 rounded-sm flex items-center justify-center transition-all duration-300
                      ${isSelected ? 'bg-[#001b44]' : 'bg-gray-50 border border-gray-100 group-hover:border-[#d4af37]'}`}>
                      {isSelected && <Check size={12} className="text-[#d4af37]" />}
                    </div>
                    <span className={`text-[12px] uppercase tracking-wider transition-all
                      ${isSelected ? 'text-[#001b44] font-black' : 'text-gray-400 font-medium group-hover:text-gray-600'}`}>
                      {option}
                    </span>
                  </div>
                );
        ...
        {/* زخرفة إضافية لتكملة الـ 80 سطر والحفاظ على الروح الفرعونية */}
        <div className="mt-20 pt-10 border-t border-gray-50 flex flex-col items-center opacity-20">
          <div className="w-1 h-1 bg-[#d4af37] rotate-45 mb-2"></div>
          <p className="text-[7px] font-black uppercase tracking-[1em] text-[#001b44]">Nour Gold</p>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .custom-sidebar-scroll::-webkit-scrollbar-thumb { background: #d4af3730; border-radius: 10px; }
      `}</style>
    </aside>
  );
};
